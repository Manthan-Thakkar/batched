CREATE OR ALTER PROCEDURE [dbo].[spCalculateDslValues]
...
BEGIN
	...
	SELECT
		...
		STRING_AGG(CONCAT([PCI].[Unit], ' - ', [PCI].[SourceColor]), ', ') WITHIN GROUP (ORDER BY [PM].[ProductNum], [PCI].[Unit], [PCI].[SourceColor]) AS [ColorByUnit_dsl]
