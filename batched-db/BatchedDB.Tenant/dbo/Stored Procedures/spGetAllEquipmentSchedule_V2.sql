CREATE PROCEDURE [dbo].[spGetAllEquipmentSchedule_V2]
	@PageNumber				AS INT = 1,
...
BEGIN
	...
	DECLARE 
		@spName								VARCHAR(100) = 'spGetAllEquipmentSchedule_V2',
...
SET @blockName = 'timecard'; SET @startTime = GETDATE();
	...
	SET @blockName = 'tasktime'; SET @startTime = GETDATE();
		...
		GROUP BY TTDP.DependentTicketId, TTD.TicketId, TTD2.TaskName
        ...
        FROM
			...
			LEFT JOIN #FinalTicketDeps td with (nolock)
				ON (tt.TicketId = td.TicketId) and tt.TaskName = 
				case when tm.Source='Radius' then tt.TaskName else td.TaskName end
	...
	SET @blockName = 'Tickets in Current Page'; SET @startTime = GETDATE();
		...
		WorkcenterMaterialTicketCategory int, IsCompletingOnTime int, HasPreviousTaskPartiallyRan bit, IsFirstDay bit, WIPValue REAL);
...
INSERT @__ErrorInfoLog EXEC spLogCreator @CorelationId, @TenantId, @SPName, @IsError, @startTime, @maxCustomMessageSize, @blockName OUTPUT, @warningStr OUTPUT, @errorStr OUTPUT, @infoStr OUTPUT
	...
	INSERT @__ErrorInfoLog EXEC spLogCreator @CorelationId, @TenantId, @SPName, @IsError, @startTime, @maxCustomMessageSize, @blockName OUTPUT, @warningStr OUTPUT, @errorStr OUTPUT, @infoStr OUTPUT
	SET @blockName = 'Ticket Value Streams'; SET @startTime = GETDATE();

    SELECT DISTINCT sr.EquipmentId, sr.SourceTicketId
	INTO #equipmentTickets
	FROM ScheduleReport sr with (nolock)
	INNER JOIN EquipmentMaster em with (nolock) ON sr.EquipmentId = em.ID
	WHERE ((SELECT Count(1) FROM @userAssignedFacilities) = 0 OR em.FacilityId IN (SELECT field FROM @userAssignedFacilities)) 

	SELECT 
		DISTINCT em.ID AS EquipmentId, 
		em.Name AS EquipmentName, 
		evs.ValueStreamId as ValueStreamId, 
		vs.Name AS ValueStreamName, 
		WorkcenterTypeId AS WorkcenterTypeId, 
	...
	LEFT JOIN ValueStream vs with (nolock)
	ON evs.ValueStreamId = vs.Id
	WHERE ((SELECT Count(1) FROM @userAssignedFacilities) = 0 OR em.FacilityId IN (SELECT field FROM @userAssignedFacilities)) 
	...
	SET @blockName = 'ValueStreams'; SET @startTime = GETDATE();
	----------- ValueStreams
	Select DISTINCT vs.Id  As ValueStreamID, vs.Name AS ValueStreamName, em.FacilityId as FacilityId, em.Id as EquipmentId, em.WorkcenterTypeId as WorkcenterId, 'tbl_valueStreams' AS __dataset_tableName  
	from #schedule s 
	inner join EquipmentMaster em with (nolock) ON em.id = s.equipmentid
	inner join EquipmentValueStream evs with (nolock) ON evs.EquipmentId = em.ID
	join ValueStream vs with (nolock) ON vs.Id = evs.ValueStreamId
	...
	from EquipmentMaster with (nolock)
	...
	SELECT 
		[Status],
		CASE
			WHEN (ExpiryTimeStamp <= GetUTCDATE()) THEN CAST(1 AS bit)
			ELSE CAST(0 AS bit)
		END AS isExpired,
		'tbl_scheduleRunStatus' AS __dataset_tableName
	FROM scheduleRunStatus with (nolock)
	...
	SET @blockName = 'TicketDependency projection'; SET @startTime = GETDATE();
	...
	INNER JOIN TaskInfo TI with (nolock)
	...
	SET @blockName = 'EnableAutomaticStockAvailability projection'; SET @startTime = GETDATE();
	----- EnableAutomaticStockAvailability projection
	SELECT 
	CASE WHEN @IsStockAvailabilityEnabled = 1 THEN 'True' ELSE 'False' END as isAutomaticStockAvailabilityEnabled,
	'tbl_isAutomaticStockAvailabilityEnabled' AS __dataset_tableName

	INSERT @__ErrorInfoLog EXEC spLogCreator @CorelationId, @TenantId, @SPName, @IsError, @startTime, @maxCustomMessageSize, @blockName OUTPUT, @warningStr OUTPUT, @errorStr OUTPUT, @infoStr OUTPUT
	

	SET @blockName = 'EnableMultiFacilityScheduling projection'; SET @startTime = GETDATE();
	...
	SET @blockName = 'Equipment tickets projection'; SET @startTime = GETDATE();
	SELECT 
	*,
	'tbl_equipmentTickets' AS __dataset_tableName
	FROM #equipmentTickets

	INSERT @__ErrorInfoLog EXEC spLogCreator @CorelationId, @TenantId, @SPName, @IsError, @startTime, @maxCustomMessageSize, @blockName OUTPUT, @warningStr OUTPUT, @errorStr OUTPUT, @infoStr OUTPUT


	SET @blockName = 'Filter data projection'; SET @startTime = GETDATE();
	SELECT 
	*,
	'tbl_filterData' AS __dataset_tableName
	FROM #filterData
	...
	DROP TABLE IF EXISTS #equipmentTickets;
