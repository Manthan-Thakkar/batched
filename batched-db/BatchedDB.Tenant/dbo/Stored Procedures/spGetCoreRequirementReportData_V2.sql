CREATE PROCEDURE [dbo].[spGetCoreRequirementReportData_V2]
	@PageNumber AS INT = 1,
...
BEGIN 
		...
		select distinct TM.Id as TicketId into #TicketsInSchedule from #LastTaskSchedule LS inner join TicketMaster TM on LS.SourceTicketId = TM.SourceTicketId
		...
		--- Ticket Fields 
			...
			tempEVS.Valuestreams as Valuestreams,
		...
		LEFT JOin TicketShipping TS on TS.TicketId = TM.ID
		LEFT join TicketDimensions TD on Td.TicketId = TM.ID
		LEFT join EquipmentMaster EM on LD.EquipmentId = EM.ID
		LEFT JOIN TicketScore TSC on TSC.TicketId = TM.ID
		LEFT JOIN TicketPreProcess TPP on TPP.TicketId = TM.ID
		...
		LEFT JOIN #equipmentValueStreams tempEVS on tempEVS.EquipmentId = EM.ID
	...
	where 
		...
		Where 
				...
				OR TicketNumber IN (SELECT field FROM @sourceTicketNumbers))
				 ...
				 AND  ((SELECT Count(1) FROM @workcenters) = 0 OR  WorkCenterId IN (SELECT field FROM @workcenters))
...
END
		...
		SELECT EquipmentId As EquipmentId, MAX(EquipmentName) as EquipmentName , 'tbl_equipments' AS __dataset_tableName
		from #LastTaskData 
		Where 
				((SELECT Count(1) FROM @sourceTicketNumbers) = 0
				OR TicketNumber IN (SELECT field FROM @sourceTicketNumbers))
		...
		---------WorkCenters
		SELECT Distinct WorkCenterId as WorkCenterId, WorkCenter AS WorkCenterName, 'tbl_workcenters' AS __dataset_tableName
		from #LastTaskData 
		Where 
				((SELECT Count(1) FROM @sourceTicketNumbers) = 0
				OR TicketNumber IN (SELECT field FROM @sourceTicketNumbers))
		...
		---------Facilities
		SELECT Distinct FacilityId as FacilityId, FacilityName AS FacilityName, 'tbl_facilities' AS __dataset_tableName
		from #LastTaskData 
		Where 
				((SELECT Count(1) FROM @sourceTicketNumbers) = 0
				OR TicketNumber IN (SELECT field FROM @sourceTicketNumbers))
		...
		SELECT distinct VS.Id  As ValueStreamID, VS.Name AS ValueStreamName, 'tbl_valueStreams' AS __dataset_tableName
			FROM #LastTaskData LTD
			JOIN EquipmentValueStream EVS on LTD.EquipmentId = EVS.EquipmentId
			JOIN ValueStream VS on EVS.ValueStreamId = VS.Id
			Where 
			((SELECT Count(1) FROM @sourceTicketNumbers) = 0
				OR TicketNumber IN (SELECT field FROM @sourceTicketNumbers))
				AND  ((SELECT Count(1) FROM @equipments) = 0 OR  LTD.EquipmentId IN (SELECT field FROM @equipments))
				AND  ((SELECT Count(1) FROM @workcenters) = 0 OR  WorkCenterId IN (SELECT field FROM @workcenters))
				AND  ((SELECT Count(1) FROM @facilities) = 0 OR  FacilityId IN (SELECT field FROM @facilities))
