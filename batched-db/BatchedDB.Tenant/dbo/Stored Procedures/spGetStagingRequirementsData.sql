CREATE PROCEDURE [dbo].[spGetStagingRequirementsData]
	@PageNumber				AS INT = 1,
	...
	@stagingComponentNames  AS UDT_SINGLEFIELDFILTER readonly,
...
BEGIN
	...
	DECLARE 
		@spName								VARCHAR(100) = 'spGetStagingRequirementsData',
...
SET @blockName = 'timecard'; SET @startTime = GETDATE();
	...
	SET @blockName = 'partiallyRanPreviousTask';
	...
	SELECT
		...
		STRING_AGG(SR.Name,', ') AS StagingRequirements
	INTO #WorkcenterStagingRequirements
	FROM
		StagingRequirementGroup SRG
		INNER JOIN StagingRequirement SR ON SR.Id = SRG.StagingRequirementId
	...
	SELECT
		...
		TPP.ProofStatus as ProofStatus,
	...
	FROM #schedule s
		...
		LEFT JOIN #WorkcenterStagingRequirements WSR ON EM.WorkcenterTypeId = WSR.WorkcenterTypeId
	...
	WHERE
			...
			AND ((SELECT Count(1) FROM @components) = 0 OR EXISTS (SELECT 1
		FROM @Components AS C
		WHERE WSR.StagingRequirements like '%' + C.Field + '%'))
	...
	--------CREATE TABLE SCRIPT--------------
		...
		Shape nvarchar(4000), StockTicketType smallint, ArtWorkComplete bit, InkReceived bit,ProofStatus NVARCHAR(4000), ProofComplete bit, PlateComplete bit,
		...
		IsPinned bit, PinType nvarchar(24), LockType nvarchar(24), LockStatus varchar(8), IsOnPress bit, ManuallyScheduled int, FeasibilityOverride bit,
		...
		StagingRequirements nvarchar(4000),WorkcenterMaterialTicketCategory int, IsCompletingOnTime int, IsFirstDay bit, WIPValue REAL, HasPreviousTaskPartiallyRan bit,
	...
	if(@SortingColumn <> 'default' AND (NOT EXISTS(SELECT * FROM @stagingComponentNames WHERE @SortingColumn LIKE '%'+Field+'%')))
	BEGIN
...
INSERT @__ErrorInfoLog EXEC spLogCreator @CorelationId, @TenantId, @SPName, @IsError, @startTime, @maxCustomMessageSize, @blockName OUTPUT, @warningStr OUTPUT, @errorStr OUTPUT, @infoStr OUTPUT
	...
	---- Default sorting
		IF(@SortingColumn = 'default' OR EXISTS(SELECT * FROM @stagingComponentNames WHERE @SortingColumn LIKE '%'+Field+'%'))
		BEGIN
		INSERT INTO #sortedPaginatedReport
			SELECT *,''
		FROM #schedulereportdetail
	...
	SET @blockName = 'Get Staging report data'; SET @startTime = GETDATE();

	SELECT *, 'tbl_StagingRequirementReport' AS __dataset_tableName FROM #sortedPaginatedReport ORDER BY RowNumber
	...
	Select count(1) as TotalCount,'tbl_StagingRequirementReport_count' AS __dataset_tableName	
	from #schedulereportdetail s
	...
	SET @blockName = 'StagingRequirements projection'; SET @startTime = GETDATE();
	----- Staging requirements projection
	DECLARE @cols AS NVARCHAR(MAX),
        @query AS NVARCHAR(MAX);

	SELECT @cols = STRING_AGG(CONCAT('SELECT TicketId, Taskname, ''', column_name, ''' as [StagingNameKey], ', column_name, ' as [IsStaged], ''tbl_ticketTaskStagingInfo'' AS __dataset_tableName  FROM TicketTaskStagingInfo'), ' UNION ALL ')
	...
	DROP TABLE IF EXISTS #WorkcenterStagingRequirements
