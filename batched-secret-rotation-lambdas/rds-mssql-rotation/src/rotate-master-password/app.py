...
def lambda_handler(event, context):
    ...
    if "RotationEnabled" in metadata and not metadata['RotationEnabled']:
        logger.error("A secret is not enabled for rotation.")
        raise ValueError("Secret %s is not enabled for rotation" % arn)
    versions = metadata['VersionIdsToStages']
    if token not in versions:
        logger.error("A secret version for rotation is missing a stage. Check configuration and rotation setup.")
        raise ValueError("Secret version %s has no stage for rotation of secret %s." % (token, arn))
    if "AWSCURRENT" in versions[token]:
        logger.info("Secret version for rotation is already set as AWSCURRENT. No action taken.")
        return
    elif "AWSPENDING" not in versions[token]:
        logger.error("Secret version for rotation is not set as AWSPENDING. Check configuration and rotation setup.")
    ...
    if step == "createSecret":
        ...
        logger.error("lambda_handler: Invalid step parameter for secret rotation request.")
...
def finish_secret(service_client, arn, token):
    ...
    After completing the rotation, it sends a message to SQS to trigger parameter updates.
    ...
    # Send message to SQS to trigger parameter updates
    _send_parameter_update_message(arn, token)


def _send_parameter_update_message(arn, token):
    """Send message to SQS queue to trigger parameter updates
    
    Args:
        arn (string): The secret ARN
        token (string): The ClientRequestToken associated with the secret version
    """
    sqs_queue_url = os.environ.get('PARAMETER_UPDATE_QUEUE_URL')
    if not sqs_queue_url:
        logger.warning("PARAMETER_UPDATE_QUEUE_URL not set, skipping parameter updates")
        return
    
    try:
        # Get the new password from the secret
        service_client = boto3.client('secretsmanager')
        secret_dict = get_secret_dict(service_client, arn, "AWSCURRENT", token)
        new_password = secret_dict['password']
        
        # Get parameter names from environment variable
        parameter_names_str = os.environ.get('PARAMETER_NAMES', '')
        parameter_names = [name.strip() for name in parameter_names_str.split(',') if name.strip()]
        
        if not parameter_names:
            logger.warning("PARAMETER_NAMES not set, skipping parameter updates")
            return
        
        # Send message to SQS
        sqs = boto3.client('sqs')
        
        # Send individual message for each parameter
        total_parameters = len([p for p in parameter_names if p])
        for i, parameter_name in enumerate(parameter_names):
            if not parameter_name:
                continue
                
            # Check if this is the last parameter
            is_last_parameter = (i == total_parameters - 1)
            ...
            logger.info(f"Sent parameter update message for {parameter_name} (last: {is_last_parameter})")
        
        logger.info(f"Sent {total_parameters} parameter update messages to SQS")
        
    except Exception as e:
        logger.error(f"Failed to send parameter update message: {e}")
        # Don't raise - parameter updates are optional and shouldn't fail the rotation
...
def connect_and_authenticate(secret_dict, port, dbname, use_ssl):
    ...
    try:
        conn = pymssql.connect(server=secret_dict['host'],
                               ...
                               login_timeout=5,
                               as_dict=True)
        logger.info("Successfully established %s database connection.", "SSL/TLS" if use_ssl else "non SSL/TLS")
        return conn
