...
import redis
import urllib3
current_month_year = (datetime.utcnow() - timedelta(hours=int(os.environ['ALGO_ES_DURATION'][:-1]))).strftime('%Y.%m')
    ...
    if 'tenantId' not in body:
        ...
        try:
            if body['getFailureDetails'] == False and body['hardReload'] == False:
                try:
                    return {
                        'statusCode': 200,
                        'body': json.dumps({'healthy': redis_get_data(conn,TenantId=body['tenantId'],WorkflowKey='AlgoWeb')})
                    }
        ...
        if response != True:
            ...
            if "No Latest CID Found" in error_message:
                error_message = "Period Schedule Generation missing"
            redis_put_data(
                conn,
...
def redis_put_data(r,TenantId,WorkflowKey,WorkflowValue,ErrorMessage=None):
    ...
    history_data = r.get("AlgoWebHistoricData-"+TenantId)
    if history_data:
    ...
    r.set("AlgoWebHistoricData-"+TenantId, json.dumps(history_list))
...
def redis_get_and_put_general_historic_data(r,TenantId):
    list_data = r.get("AlgoWebHistoricData-"+TenantId)
    ...
    for attempt in range(max_retries):
        try:
            with r.pipeline() as pipe:
                ...
                data_to_store['AlgoWeb_SuccessCount'] = success_count
...
def get_status_for_tenant(tenant_id,body):
    ...
    print(str(cid_list)+"For TID "+ str(tenant_id))
    return get_status(cid_list,body)

def get_cid(tenant_id):
    conn = es_connection()
    query = {"_source":"false","size":0,"sort":[{"@timestamp":{"order":"desc"}}]\
            ,"query":{"bool":{"must":[{"term":{"log_processed.Properties.app_name.keyword":"algo-web"}},{"term":{"log_processed.msg.keyword":"run-schedule"}},{"term":{"log_processed.Properties.tid.keyword":tenant_id}}\
            ,{"range":{"@timestamp":{"gte":"now-"+str(os.environ['ALGO_ES_DURATION'])+"/d","lte":"now/d"}}}]}}\
            ,"aggs":{"unique_cids":{"terms":{"field":"log_processed.Properties.cid.keyword","size":1,"order":{"latest_event":"desc"}},"aggs":{"latest_event":{"max":{"field":"@timestamp"}}}}}}

    resp = conn.search(index="r-scheduler-"+current_month_year+"*",body=query)
    print(resp)
...
def get_status(cid,body):
    print(cid)
    if cid == []:
        return "No Latest CID Found in Past "+str(os.environ['ALGO_ES_DURATION'])
    conn = es_connection()
    query = {"_source":"false","size":0,"sort":[{"@timestamp":{"order":"desc"}}],"query":{"bool":{"must":[{"terms":{"log_processed.Properties.cid.keyword":cid}},{"term":{"log_processed.Properties.app_name.keyword":"algo-web"}},{"term":{"log_processed.msg.keyword":"run-schedule"}},{"range":{"@timestamp":{"gte":"now-"+str(os.environ['ALGO_ES_DURATION'])+"/d","lte":"now/d"}}}]}},"aggs":{"unique_status":{"terms":{"field":"log_processed.Properties.status.keyword","size":100,"order":{"latest_event":"desc"}},"aggs":{"latest_event":{"max":{"field":"@timestamp"}}}}}}
    resp = conn.search(index="r-scheduler-"+current_month_year+"*",body=query)
...
def get_failure_message(cid):
    conn = es_connection()
    query = {"_source":["log_processed.msg"],"size":1,"sort":[{"@timestamp":{"order":"desc"}}],"query":{"bool":{"must":[{"terms":{"log_processed.Properties.cid.keyword":cid}},{"term":{"log_processed.Properties.app_name.keyword":"algo-web"}},{"wildcard":{"log_processed.msg":"*error*"}},{"range":{"@timestamp":{"gte":"now-"+str(os.environ['ALGO_ES_DURATION'])+"/d","lte":"now/d"}}}]}}}
    resp = conn.search(index="r-scheduler-"+current_month_year+"*",body=query)['hits']['hits'][0]['_source']
    return resp
