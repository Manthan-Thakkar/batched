    ...
    if 'tenantId' not in body:
        ...
        try:
            if body['getFailureDetails'] == False and body['hardReload'] == False:
                try:
                    ...
                    return {
                        'statusCode': 200,
                        'body': json.dumps({'healthy': redis_get_data(conn,TenantId=body['tenantId'],WorkflowKey='WorkflowJob')})
                    }
                except Exception as e:
                    print(f'unable to fetch from redis. fetching from es. Error: {e}')
        ...
        if response != True:
            ...
            if "No Latest CID Found" in error_message:
                error_message = "Data Transformation missing"
            redis_put_data(
                conn,
                TenantId=body['tenantId'],
                WorkflowKey='WorkflowJob',
                WorkflowValue='false',
...
def redis_put_data(r,TenantId,WorkflowKey,WorkflowValue,ErrorMessage=None):
    ...
    history_data = r.get("WorkflowJobHistoricData-"+TenantId)
    if history_data:
    ...
    r.set("WorkflowJobHistoricData-"+TenantId, json.dumps(history_list))
    # r.ltrim("WorkflowJobHistoricData-"+TenantId, 0, 9)
...
def redis_get_and_put_general_historic_data(r,TenantId):
    list_data = r.get("WorkflowJobHistoricData-"+TenantId)
    ...
    for attempt in range(max_retries):
        try:
            with r.pipeline() as pipe:
                ...
                data_to_store['WorkflowJob_SuccessCount'] = success_count
...
def get_status_for_tenant(tenant_id,body):
    # First, check RDSDataImport (prerequisite step)
    rds_import_cid_list = get_rds_import_cid(tenant_id)
    print(f"RDS CID: {rds_import_cid_list}")
    
    if not rds_import_cid_list:
        # No RDS import found - workflow hasn't started
        return "No Latest CID Found in Past "+str(os.environ['ES_DURATION'])
    
    # Check if RDS import has exceptions
    rds_status = check_rds_import_exceptions(rds_import_cid_list, body)
    if rds_status != True:
        # RDS has exceptions, return the failure
        return rds_status
    
    # RDS succeeded, now check post_data_ingestion_workflow
    cid_list = get_cid(tenant_id)
    print(f"Post ingestion CID: {cid_list}")
    return get_status(cid_list,body)

def get_rds_import_cid(tenant_id):
    conn = es_connection()
    query = {"_source":"false","size":0,"sort":[{"@timestamp":{"order":"desc"}}]\
            ,"query":{"bool":{"must":[{"term":{"app_name.keyword":"RDSDataImport"}},{"term":{"tid.keyword":tenant_id}}\
...
def get_cid(tenant_id):
    conn = es_connection()
    query = {"_source":"false","size":0,"sort":[{"@timestamp":{"order":"desc"}}]\
            ,"query":{"bool":{"must":[{"term":{"app_name.keyword":"post_data_ingestion_workflow"}},{"wildcard":{"category.keyword":"*status*"}},{"term":{"tid.keyword":tenant_id}}\
            ,{"range":{"@timestamp":{"gte":"now-"+str(os.environ['ES_DURATION'])+"/d","lte":"now/d"}}}]}}\
            ,"aggs":{"unique_cids":{"terms":{"field":"cid.keyword","size":1,"order":{"latest_event":"desc"}},"aggs":{"latest_event":{"max":{"field":"@timestamp"}}}}}}
...
def check_rds_import_exceptions(cid, body):
    """Check if RDS import has exceptions"""
    print(f"Checking RDS exceptions for CID: {cid}")
    
    conn = es_connection()
    query = {"_source":"false","size":0,"sort":[{"@timestamp":{"order":"desc"}}],"query":{"bool":{"must":[{"terms":{"cid.keyword":cid}},{"term":{"app_name.keyword":"RDSDataImport"}},{"term":{"type.keyword":"exception"}},{"range":{"@timestamp":{"gte":"now-"+str(os.environ['ES_DURATION'])+"/d","lte":"now/d"}}}]}},"aggs":{"exception_count":{"value_count":{"field":"type.keyword"}}}}
    resp = conn.search(index="batched-logs-"+current_month_year+"*",body=query)
    
    exception_count = resp['aggregations']['exception_count']['value']
    print(f"RDS CID - {cid} has exception_count - {exception_count}")
    ...
    if exception_count > 0:
        try:
            ...
            return str(get_rds_exception_message(cid))
    else:
        return True  # No exceptions, continue to post_data_ingestion_workflow check
...
def get_status(cid,body):
    ...
    if cid == []:
        return " No Latest CID Found in Past "+str(os.environ['ES_DURATION'])
    conn = es_connection()
    query = {"_source":"false","size":0,"sort":[{"@timestamp":{"order":"desc"}}],"query":{"bool":{"must":[{"terms":{"cid.keyword":cid}},{"term":{"app_name.keyword":"post_data_ingestion_workflow"}},{"wildcard":{"category.keyword":"*status*"}},{"range":{"@timestamp":{"gte":"now-"+str(os.environ['ES_DURATION'])+"/d","lte":"now/d"}}}]}},"aggs":{"unique_status":{"terms":{"field":"status.keyword","size":100,"order":{"latest_event":"desc"}},"aggs":{"latest_event":{"max":{"field":"@timestamp"}}}},"unique_type":{"terms":{"field":"type.keyword","size":100,"order":{"latest_event":"desc"}},"aggs":{"latest_event":{"max":{"field":"@timestamp"}}}}}}
    resp = conn.search(index="batched-logs-"+current_month_year+"*",body=query)

    status_values = list(map(lambda i: i['key'], resp['aggregations']['unique_status']['buckets']))
    type_values = list(map(lambda i: i['key'], resp['aggregations']['unique_type']['buckets']))
    print("CID - "+str(cid)+ " has status_values - "+str(status_values) + " and type_values - " + str(type_values))
    
    if 'failure' in status_values or 'exception' in type_values:
        try:
            if body['getFailureDetails'] == False:
                return False
        except:
            return str(get_failure_message(cid, type_values))
    else:
        return True
    
def get_rds_exception_message(cid):
    """Get exception message for RDSDataImport"""
    conn = es_connection()
    query = {"_source":["message"],"size":1,"sort":[{"@timestamp":{"order":"desc"}}],"query":{"bool":{"must":[{"terms":{"cid.keyword":cid}},{"term":{"app_name.keyword":"RDSDataImport"}},{"term":{"type.keyword":"exception"}},{"range":{"@timestamp":{"gte":"now-"+str(os.environ['ES_DURATION'])+"/d","lte":"now/d"}}}]}}}
    resp = conn.search(index="batched-logs-"+current_month_year+"*",body=query)['hits']['hits'][0]['_source']
    return resp

def get_failure_message(cid, type_values):
    conn = es_connection()
    
    # If there's an exception type, fetch exception message first (more specific error)
    if 'exception' in type_values:
        query = {"_source":["message"],"size":1,"sort":[{"@timestamp":{"order":"desc"}}],"query":{"bool":{"must":[{"terms":{"cid.keyword":cid}},{"term":{"app_name.keyword":"post_data_ingestion_workflow"}},{"term":{"type.keyword":"exception"}},{"range":{"@timestamp":{"gte":"now-"+str(os.environ['ES_DURATION'])+"/d","lte":"now/d"}}}]}}}
        try:
            resp = conn.search(index="batched-logs-"+current_month_year+"*",body=query)['hits']['hits'][0]['_source']
            return resp
        except:
            pass  # Fall through to status:failure check
    
    # Otherwise, fetch status:failure message
    query = {"_source":["message"],"size":1,"sort":[{"@timestamp":{"order":"desc"}}],"query":{"bool":{"must":[{"terms":{"cid.keyword":cid}},{"term":{"app_name.keyword":"post_data_ingestion_workflow"}},{"wildcard":{"category.keyword":"*status*"}},{"term":{"status.keyword":"failure"}},{"range":{"@timestamp":{"gte":"now-"+str(os.environ['ES_DURATION'])+"/d","lte":"now/d"}}}]}}}
    resp = conn.search(index="batched-logs-"+current_month_year+"*",body=query)['hits']['hits'][0]['_source']
    return resp
