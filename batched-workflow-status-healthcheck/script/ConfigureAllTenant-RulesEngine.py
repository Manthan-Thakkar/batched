...
SCHEDULER_GROUP_NAME = 'rules-engine-healthcheck-schedules'
SQS_QUEUE_ARN = 'arn:aws:sqs:us-east-2:<AccountID>:RulesEngineStep-HealthCheck.fifo'  # Update with actual ARN
...
def get_all_tenants():
    ...
    selected_columns_df.to_csv('RulesEngineData.csv', index=False)
...
def create_schedule(tenant_name, tenant_id):
    """Create EventBridge schedule for a tenant's rules engine health check"""
    ...
    try:
        response = scheduler_client.create_schedule(
            ...
            Target={
                ...
                'SqsParameters': {
                    'MessageGroupId': 'rulesengine-healthcheck-group'  # Single group ID for sequential processing
                }
            },
            State='ENABLED',
...
def main():
    print("=" * 80)
    print("RulesEngine Health Check - EventBridge Schedule Configuration")
    print("=" * 80)
